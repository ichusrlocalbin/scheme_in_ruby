...
関数適用@<tt>{_apply_primitive_fun}では、Rubyの関数@<tt>{_apply_primitive_fun(fun, args)}を呼び出しています。ここでは、与えられたシンボルに対応する組み込み関数を呼び出しています。参考として、今まで利用していた@<tt>{apply_primitive_fun}も記載します。少しややこしいですが、@<tt>{_apply_primitive_fun(fun, args)}をはじめとする関数を定義することにより、μSchemeR上で呼び出すことが可能となるため、これから作成する@<i>{p}schemeを動作させるときにも使用することができるようになります。
...
最初は、リストを扱う式の拡張です。これらは@<tt>{car}と@<tt>{cdr}の組み合わせから成ります。リストに適応する順に@<tt>{car}であれば @<tt>{a}を、@<tt>{cdr}であれば @<tt>{d}を左から追加し、最初と最後をそれぞれcとrとして挟みます。例えば、@<tt>{cdar}は@<tt>{car}したリストを@<tt>{cdr}して返す関数です。
...
@<tt>{map2} は、与えられた二つのリストの要素を与えられた関数へ適応し、その結果をリストとして返す関数です。@<tt>{map2}は@<tt>{map}を使って次のように書くことができます。
...
ここで使った@<tt>{zip}は二つのリストの対応する要素同士をペアとしてそれらのペアを返す関数です。
...
この関数を使って、上の関数を書き直すことができます。例えば、@<tt>{map}は次のように書き直すことが出来ます。ここでは、@<tt>{_fun}を与えられたリスト要素をmap関数の引数として渡された関数@<tt>{fun}に適用し、その結果と演算結果@<tt>{accum}のconsペアを返す関数に束縛しています。この関数と、空リストを初期演算結果としたものと、与えられたリストとを引数にして@<tt>{fold}関数を呼び出しています。注意すべきは、最後に@<tt>{reverse}関数で結果を並び替えている点です。リストの先頭要素から関数適応され、consペアが作成されていくことから、引数のリストと$<tt>{fold}の結果のリストとの順番が逆になるためです。頭の中で動きを理解できたら、ぜひ、@<tt>{append}, @<tt>{reverse}, @<tt>{zip}を@<tt>{fold}を使って書き換えてみて下さい。高階関数を使う良い演習になると思います。

このように、高階関数を扱うことが出来るプログラミング言語では、関数の中に同じ構造を見つけたら、それを抽象化した関数を作成し書き換えることが出来ます。抽象化していく中で、何が本質的な計算なのかが見えてくることもありますので、どんどん試してみてください。
...
大変お待たせしました。@<i>{p}schemeを実装しましょう。と言っても、実はほとんどやることはありません。後は、μSchemeRのコードを見ながら移植していけば良いだけです。一気にコードを書いてしまいます。
...
いかがでしょうか。末尾に参考としてμSchemeRのコードをつけますので、それと見比べて見てください。ほとんど同じコードであることが分かると思います(μSchemeRのコードはこれを見越して書いたので当然なのですが)。言い換えれば、この章を読む前にすでにμSchemeRのコードで関数型言語のプログラミング方法について学んでいたことになります。
