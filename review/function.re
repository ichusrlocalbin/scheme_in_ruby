...
λ式を評価するときに、評価時の環境も合わせて持っておくことで、これを実現できます。λ式を評価した時にその結果として、λ式とその時の環境をペア([@<m>{lambda() x}, @<tt>{[{x:2\}]}])で持ち、@<tt>{:fun}はこれを値として束縛します。その後@<tt>{:x}が1を束縛すると、環境は@<tt>{[{x:1\}, {x:2\}]}となります。ただし、@<tt>{:fun}を関数適用する際には、先ほどのペアで作成した環境@<tt>{[{x:2\}]}を利用してλ式を評価することで@<tt>{x}の値は2となります。
