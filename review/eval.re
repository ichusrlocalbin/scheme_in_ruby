...
関数適用は、引数の評価値(Ruby上の値になります)を(Ruby上の)関数へ適用しています。@<tt>{fun_val.call(*args)}は、@<tt>{fun_val}というRuby上の関数を引数@<tt>{args}で呼び出します。@<tt>{*}は、可変長引数に対応しており、@<tt>{args}が@<tt>{[1, 2]}の場合fun_val.call(1, 2)と展開され、@<tt>{args}が@<tt>{[1, 2, 3]}の場合@<tt>{fun_val.call(1, 2, 3)}と展開されます。
...
実際に、@<tt>{[:+, 1, 2]}を評価するときの動きを@<img>{uschemer_ruby}を見ながら追ってみましょう。与えるプログラムはリストですので、まず先頭の要素@<tt>{:+}を評価し、@<m>{lambda{|x, y| x + y\}}を値として得ます。これは、二つの引数を足す(Ruby上の)関数です。次に@<tt>{1}, @<tt>{2}を評価し、それぞれ(Ruby上での)1, 2を得ます。これを(Ruby上で)適用することで、3を得ます。この値はputsを使って、Ruby上のプログラムで表示することができます@<fn>{fn5}。
...
//footnote[fn2][@<tt>{eval}としないで@<tt>{_eval}としているのは、Rubyの組み込み関数として@<tt>{eval}が定義されているためです。気になる人は、Rubyのmodule機能を使って名前空間を分け、@<tt>{_eval}を@<tt>{eval}として定義しなおして下さい。またその際は、4章の@<tt>{parse}で使われる@<tt>{eval}は@<tt>{Kernel::eval}に置き換えて下さい。]
