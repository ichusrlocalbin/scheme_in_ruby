...
次にプログラムとは何か、という観点で上の文章を言い換えてみます。プログラムは計算するための手順を記したものと言われていますが、本文書ではこれとは異なる方法で定義します。すなわち、プログラムが与えられたとき、それをどう実行するかを定義します。今回は、自然言語で書かれた仕様書ではなく、Rubyのプログラムによって、何をプログラムとして受け入れ、それがどう実行されるのかを定めます。これにより、自然言語に含まれるあいまいさがなく、プログラムとは何かを定めることができます。
...
また、要素は「@<tt>{,}」で区切り、それらを「@<tt>{[}」と「@<tt>{]}」で囲み、リストを表します。この形式は、Rubyで配列としてそのまま扱えるため、計算機側で余計な処理を考えなくて良くなります。また、記号である@<tt>{+}や変数である@<tt>{x}や予約語@<tt>{if}などは、単語のはじめに@<tt>{:}をつけてそれぞれ@<tt>{:+}、@<tt>{:x}、@<tt>{:if}と記します。この表記法もRubyでそのままシンボルとして扱えるために導入しています。
...
式がリストであった場合、最初の要素を関数として、残りを引数として、それぞれを評価してその値を求めます。求めた関数に求めた引数の値を適用(apply)してその結果を@<tt>{_eval}の結果とします@<fn>{fn3}。一方、リストでない場合、数字であれば即値として扱い数字そのものを返します。例えば@<tt>{2}は2を返します。そうでなければ、組み込み関数とみなし、それに関連付けられた(Ruby上での)関数を返します。
...
//note[プログラムコードと値の区別]{
「@<tt>{2}は2を返します」という文章で数字のフォントが違っていることに気づきましたか? @<tt>{2}は(現在考えているμSchemeR)プログラムの、2は(Ruby上での)数値の2を表します。こう記載することによって、プログラムとその評価された値とを区別します。@<img>{uschemer_ruby}を確認してください。
//}
...
組み込み関数は、関数名をキーに、関数本体をその値としたハッシュで保有します。関連付けられている関数はRuby上での関数です。組み込み関数の評価は、関数名に関連付けられた関数を値として返します。
...
@<tt>{car}はリストの先頭の要素を、@<tt>{cdr}は先頭の要素以降のリストを取得する関数です。この名前は奇妙@<fn>{fn4}ですが、Schemeで使われている名前ですので我慢してください。そのうち慣れてくるでしょう。
...
関数適用は、引数の評価値(Ruby上の値になります)を(Ruby上の)関数へ適用しています。@<tt>{fun_val.call(*args)}は、@<tt>{fun_val}というRuby上の関数を引数@<tt>{args}で呼び出します。@<tt>{*}は、可変長引数に対応しており、@<tt>{args}が@<tt>{[1, 2]}の場合@<tt>{fun_val.call(1, 2)}と展開され、@<tt>{args}が@<tt>{[1, 2, 3]}の場合@<tt>{fun_val.call(1, 2, 3)}と展開されます。
...
実際に、@<tt>{[:+, 1, 2]}を評価するときの動きを@<img>{uschemer_ruby}を見ながら追ってみましょう。与えるプログラムはリストですので、まず先頭の要素@<tt>{:+}を評価し、@<i>{lambda{|x, y|  x + y\}}を値として得ます。これは、二つの引数を足す(Ruby上の)関数です。次に@<tt>{1}, @<tt>{2}を評価し、それぞれ(Ruby上での)1, 2を得ます。これを(Ruby上で)適用することで、3を得ます。この値はputsを使って、Ruby上のプログラムで表示することができます@<fn>{fn5}。
...
//footnote[fn1b][Ruby では@<tt>{:}に文字列を続けることで、それを定数のように扱えるシンボルという機能が存在します。シンボルを使わない場合、@<tt>{x}を評価するとRuby 上の変数として評価されるため、@<tt>{x}がRuby で定義されていない場合エラーになるため扱いが面倒となります。]

//footnote[fn2][@<tt>{eval}としないで@<tt>{_eval}としているのは、Rubyの組み込み関数として@<tt>{eval}が定義されているためです。気になる人は、Rubyのmodule機能を使って名前空間を分け、@<tt>{_eval}を@<tt>{eval}として定義しなおして下さい。またその際は、第4章の@<tt>{parse}で使われる@<tt>{eval}は@<tt>{Kernel::eval}に置き換えて下さい。]

//footnote[fn3][Rubyでは、関数の最後に評価した式の値が関数の返り値になります。一般的に関数型言語では@<tt>{return} @<b>{<値>}と書かずに、このような書き方をします。本書では、関数型言語の考え方に近づくようにこの記述を利用していきます。]
