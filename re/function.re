...
いずれも、引数を二つとりその和を値とする関数を用意し、その関数に引数3と2を与えて関数適用しているプログラムです。異なる点は、最初のプログラムの関数は名前を持っていません。言わば、関数の中身そのものです。もう少し詳しく説明すると、@<tt>{[:lambda, 【parameters】, 【body】}は仮引数【parameters】でボディが【body】の関数です。これを関数として、引数を与えることで、前章で学んだ組み込み関数@<tt>{:+}と同様に関数適用することができます。

このプログラムを実行するためにはどうすれば良いでしょうか。「引数の3, 2を仮引数のx, yにそれぞれ代入して、中のプログラムを評価する」と考える人が多いのではないでしょうか。おおよそ正しいのですが、考慮すべきポイントがあります。それを見ていきましょう。
...
λ式を評価するときに、評価時の環境も合わせて持っておくことで、これを実現できます。λ式を評価した時にその結果として、λ式とその時の環境をペア([@<m>{lambda() x}, [{x:2\}]}])で持ち、@<tt>{:fun}はこれを値として束縛します。その後@<tt>{:x}が1を束縛すると、環境は@<m>{[{x:1\}, {x:2\}]}となります。ただし、@<tt>{:fun}を関数適用する際には、先ほどのペアで作成した環境[{x:2}]を利用してλ式を評価することでxの値は2となります。
...
最後に、@<tt>{_eval}を変更します。式に加えて環境も引数とします。その他、導入したλ式やlet式を扱えるよう変更します。applyもλ式の関数適用を扱えるように変更します。
...
を束縛したλ式中の@<tt>{:x}はその中で値を束縛していないにも関わらず利用できる点に注意して下さい@<fn>{fn3}。これはクロージャが環境を持っているからです。
