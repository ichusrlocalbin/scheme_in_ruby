...
@<tt>{null?}は与えたリストが空リストか調べるものです。空リストは:nilで表されます。その他、後で説明するリスト用の組み込み関数も環境として定義しておきます。
...
次のように「()」を使う本来のSchemeの記述方法プログラムをRubyの文字列として入力すると、今までと同じように「[]」や「,」を使ったRubyのデータ型に変換するものを作ります。変換後のデータを評価させれば今までどおりの結果が得られますので、ユーザは「()」を使う普通のSchemeのプログラムを入力できるようになります。
...
これは、「(」「)」を「[」「]」に、変数をRubyのシンボルに置き換えるため「:」を変数の先頭に追加し、空白を「,」に置換するより実現します。
...
他に不便なところはありませんか。まだまだあるでしょう。実現していない機能は多々あります。named letやlet*などの機能を調べそ実装にトライしてみて下さい。友達に速度が遅いと言われたら、コンパイラを作りましょう。Haskellのように遅延評価でないとと言われたら、遅延評価にしてしまいましょう。他の言語のこの機能がない、と言われたら、自分で追加してしまいましょう。自分でプログラミング言語を作ったからこそ味わえるおもしろさです。大いに使い倒して下さい。
...
//footnote[fn2][実行する前にp.で示すspecial_form?とeval_special_formを追記する必要があります。]

//footnote[fn3][quoteは通常、’を使って簡易に記載できます。すなわち、(quote 1 2 3)は’(1 2 3)と同じものです。これは処理系が’を読み込んだ時に、quoteに展開することで実現されています。腕に自信のある方はぜひこの機能の実装にチャレンジしてみて下さい。Ruby 1.9から正規表現でカッコの対応付けが可能になっています。]

//footnote[fn4][pretty printの略です。]
